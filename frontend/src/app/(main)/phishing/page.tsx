...
export default function PhishingPage() {
  ...
  const loadTemplates = useCallback(async () => {
    try {
      const d = await api.json("/phishing/templates");
  ...
  const loadCaptures = useCallback(async () => {
    try {
      const d = await api.json("/phishing/captures");
  ...
  const handleLaunch = async (id: number) => {
    try {
      await api.post(`/phishing/campaigns/${id}/launch`);
      loadCampaigns();
    } catch { toast.error(t("phishing.toast.save_campaign_failed")); }
  };
                ...
                {campaigns.map((c) => {
                  ...
                  return (
                    <TableRow key={cid}>
                      ...
                      <TableCell className="px-4 py-3 sm:py-3.5 text-right">
                        {c.status === "draft" && <Button variant="ghost" size="icon-xs" onClick={() => handleLaunch(cid)} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 mr-3" title="Launch" aria-label="Launch"><Play className="w-4 h-4" /></Button>}
                        {c.status === "running" && <Button variant="ghost" size="icon-xs" onClick={() => handleStop(cid)} className="text-amber-600 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 mr-3" title="Stop" aria-label="Stop"><Square className="w-4 h-4" /></Button>}
                        {(c.status === "draft" || c.status === "completed") && <Button variant="ghost" size="icon-xs" onClick={() => handleDeleteCamp(cid)} className="text-muted-foreground hover:text-destructive" title="Delete" aria-label="Delete"><Trash2 className="w-4 h-4" /></Button>}
                      </TableCell>
                    </TableRow>
                  );
                })}
