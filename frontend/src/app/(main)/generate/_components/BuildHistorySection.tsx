...
import { CheckCircle, ChevronDown, ChevronUp, History, XCircle } from "lucide-react";
...
export default function BuildHistorySection({ refreshKey }: { refreshKey?: number }) {
  ...
  useEffect(() => {
    api.get<{ builds?: BuildHistoryEntry[]; Logs?: BuildHistoryEntry[] }>("/builds?pageSize=20")
      ...
      .catch(() => { toast.error("Failed to load build history"); })
  ...
  return (
    <div className="mt-8">
      <Button type="button" variant="ghost" onClick={() => setExpanded(!expanded)} className="flex items-center gap-x-3 mb-5 w-full text-left justify-start h-auto py-0">
        ...
        {expanded ? <ChevronUp className="w-3 h-3 text-muted-foreground transition-transform" /> : <ChevronDown className="w-3 h-3 text-muted-foreground transition-transform" />}
      </Button>
      {expanded && (
                ...
                <TableRow key={b.id}>
                  ...
                  <TableCell>
                    {b.status === "success" ? (
                      ...
                      <Badge variant="destructive" className="text-xs gap-1" title={b.error}><XCircle className="w-4 h-4" /> Failed</Badge>
                    )}
                  </TableCell>
