...
export default function CampaignsPage() {
  ...
  const loadCampaigns = async () => {
    try {
      const data = await api.get<{success: boolean; data?: Campaign[]}>(`/campaigns`);
  ...
  const loadCampaignDetail = async (id: string) => {
    try {
      const data = await api.json<{stats?: CampaignStats}>(`/campaigns/${id}`);
  ...
  useEffect(() => {
    if (selectedCampaign) loadCampaignDetail(selectedCampaign);
    ...
    (async () => {
      try {
        const [mitreJson, tplJson] = await Promise.all([
          api.get<{success: boolean; data?: CampaignMITRE}>(`/campaigns/${campaign.id}/mitre`),
          api.get<{success: boolean; data?: KillChainTemplate[]}>(`/mitre/templates`),
        ]);
              ...
              <>
                  ...
                  <div className="flex gap-1 min-w-max">
                    {PHASE_ORDER.map((phase) => {
                      ...
                      >
                          ...
                          <span className={`text-[10px] text-center leading-tight ${isCompleted ? "text-emerald-700 dark:text-emerald-400 font-medium" : isPending ? "text-muted-foreground" : "text-amber-600 dark:text-amber-400"}`}>
                            {phase.split(" ").slice(0, 2).join("\n")}
                          </span>
                          {found && (
                            <span className="text-[9px] text-muted-foreground">{found.task_count}</span>
                          )}
                          </Button>
                      );
                    })}
                ...
                {expandedPhase && phaseTasks[expandedPhase] && phaseTasks[expandedPhase].length > 0 && (
                  <div className="mt-3 p-3 rounded-lg bg-muted/50">
                    <h4 className="text-sm font-semibold mb-2">{expandedPhase} Tasks</h4>
                ...
                {(mitreData?.timeline || []).length === 0 ? (
                  ...
                  (mitreData?.timeline || []).map((event, i) => (
                    <div key={i} className="mb-3 relative">
                      ...
                      <div className="text-xs text-muted-foreground">{event.task_count} tasks</div>
                    </div>
                  ))
                )}
              </div>
              ...
              <div className="mt-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {templates.find((t) => t.name === selectedTemplate)?.steps.map((step, i) => (
                    <span key={i} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs bg-muted/50">
...
<Card className="">
                    ...
                    <TableRow key={a.agent_id}>
                      ...
                      <TableCell className="py-3 px-4 sm:py-3.5 sm:px-5 text-xs">
                        {Object.entries(a.phases || {}).map(([phase, count]) => (
                          <span key={phase} className="inline-block mr-1 mb-1 px-1.5 py-0.5 rounded text-xs bg-muted/50">
                            {phase}:{count}
                          </span>
                        ))}
                      </TableCell>
        ...
        <p className="text-center text-muted-foreground py-8">{t("campaign.loading_stats")}</p>
      )}
    </div>
  );
}
