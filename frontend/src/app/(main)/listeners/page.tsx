...
import { AlertCircle, ArrowLeftRight, Copy, Info, Pencil, Plug, Plus, Power, Trash2, X } from "lucide-react";
...
export default function ListenersPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error("Failed: " + (data.error || t("listeners.toast_unknown_error")));
      }
    } catch {
  ...
  useEffect(() => {
    loadListeners();
    api.get("/agents?page=1&pageSize=500")
        .then((d) => {
    ...
    <div className="max-w-[80rem] mx-auto pb-12 md:pb-0 animate-fade-slide-up">
      {error && (
        <div className="mb-4 px-4 py-3 bg-destructive/10 border border-destructive/20 rounded-xl flex items-center gap-3 text-sm text-destructive">
          ...
          <span>{error}</span>
          <Button variant="ghost" size="icon-sm" onClick={() => setError(null)} className="ml-auto text-destructive/50 hover:text-destructive" aria-label="Dismiss error">
            <X className="w-4 h-4" />
          </Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <Card className="rounded-xl p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t("listeners.total")}</div>
          <div className="text-2xl font-bold mt-1 text-foreground">{loading ? "..." : total}</div>
        </Card>
        <Card className="rounded-xl p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t("listeners.running")}</div>
          <div className="text-2xl font-bold mt-1 text-emerald-600">{loading ? "..." : enabledCount}</div>
        </Card>
        <Card className="rounded-xl p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t("listeners.http")}</div>
          <div className="text-2xl font-bold mt-1 text-foreground">{loading ? "..." : httpCount}</div>
        </Card>
        <Card className="rounded-xl p-4 sm:p-5">
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t("listeners.tcp")}</div>
          <div className="text-2xl font-bold mt-1 text-foreground">{loading ? "..." : tcpCount}</div>
        </Card>
        <Card className="rounded-xl p-4 sm:p-5 col-span-2 sm:col-span-1">
          <div className="text-[10px] sm:text-xs text-muted-foreground">{t("listeners.dns")}</div>
                ...
                return (
                  <TableRow key={id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <TableCell className="max-sm:hidden py-3 px-3 sm:py-4 sm:px-4">
                      <Badge variant={type === "http" ? "outline" : "success"} className="text-[11px] font-medium">{type}</Badge>
                    </TableCell>
                    <TableCell className="max-sm:hidden py-3 px-3 sm:py-4 sm:px-4">
