...
import { ArrowLeft, ArrowRight, CloudUpload, Download, Eye, File, FileText, FolderOpen, FolderTree, HardDrive, ImageIcon, Loader2, RotateCw, Search, Trash2, X } from "lucide-react";
                  ...
                  >
                    ...
                    <div className="text-left">                      <div className="text-sm font-medium text-foreground">
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">
                          {t("agents.files_free_of", { free: formatSize(drive.free), total: formatSize(drive.total) })}                          <div className="w-full bg-muted rounded-full h-1.5 mt-1">
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            ...
            {uploading && (
              <div>
                ...
                <div className="w-full bg-muted rounded-full h-2">                    <div                    className="bg-emerald-500 h-2 rounded-full transition-all duration-200"
            ...
            <DialogFooter className="flex gap-3 pt-2 sm:justify-stretch">
              ...
              >
                {uploading ? (                    <><Loader2 className="w-4 h-4" /> {t("agents.files_uploading")}</>
                ) : (
          ...
          <DialogHeader className={`px-6 py-5 ${              previewIsImage ? "bg-gradient-to-r from-pink-500 to-pink-700" : "bg-gradient-to-r from-indigo-500 to-indigo-700"
            ...
            <DialogTitle className="text-lg font-semibold text-white flex items-center justify-between">
              <span className="flex items-center gap-2">
                {                  previewIsImage ? (
                    <><ImageIcon className="w-4 h-4" /> {t("agents.files_image")} {selectedFile}</>
                  ) : (
              ...
              <span className="flex items-center gap-2">                {!previewIsImage && (
                <Button variant="ghost" size="icon-sm"
                  ...
                  title={t("agents.files_download_content")}
