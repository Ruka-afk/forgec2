...
export default memo(function ConnectionLogSection({ logs }: ConnectionLogSectionProps) {
  ...
  return (
    <SectionCard className="mb-4" title={t("agents.detail_connection_log")} icon={<History className="w-3.5 h-3.5" />}>
      <div className="max-h-64 overflow-y-auto">
        <div className="divide-y divide-border">
          {logs.map((log, i) => (
            <div key={log.id || i} className="px-4 py-2 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <Circle className={`w-1.5 h-1.5 fill-current ${log.type === "online" ? "text-success" : log.type === "offline" ? "text-destructive" : "text-primary"}`} />
                <span className="text-xs text-foreground">{log.user || t("agents.detail_log_system")}</span>
                {log.message && <span className="text-xs text-muted-foreground/70">{log.message}</span>}
              </div>
