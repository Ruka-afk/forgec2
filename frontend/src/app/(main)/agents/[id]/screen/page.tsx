...
import { ArrowLeft, Camera, Clock, Download, ImageIcon, Images, Loader2, Maximize2, Monitor, Play, RotateCw, Square, TriangleAlert, X, Zap } from "lucide-react";
...
interface ScreenshotItem {
  ...
  const statusIndicator = () => {
    if (status === "capturing") return { color: "bg-amber-400 animate-pulse", text: "Capturing...", icon: <Loader2 className="w-3 h-3 animate-spin" /> };
              ...
              <div className="bg-muted px-4 py-2.5 flex items-center justify-between border-b border-border shrink-0">                <div className="flex items-center gap-2">
                ...
                {screenshot ? (
                  ...
                  <div className="text-center text-muted-foreground/70 py-20">
                    <Monitor className="w-4 h-4" />
                    <p className="text-base font-medium">{t("agents.screen_no_screenshots")}</p>                    <p className="text-sm mt-1 text-muted-foreground/70">{t("agents.screen_start")} / {t("agents.screen_capture")}</p>                  </div>
                )}
                {status === "capturing" && (
            ...
            <Card className="p-4 shrink-0">
              <div className="text-xs text-muted-foreground mb-3 font-semibold uppercase tracking-wider flex items-center justify-between">
                <span>{t("agents.screen_controls")}</span>
                 <span className={`flex items-center gap-1.5 text-[10px] font-normal ${monitoring ? "text-emerald-500" : "text-muted-foreground/70"}`}>                  <span className={`w-1.5 h-1.5 rounded-full ${monitoring ? "bg-emerald-500 animate-pulse" : "bg-muted-foreground"}`}></span>                  {monitoring ? "LIVE" : "OFF"}
                </span>
              </div>
              <div className="space-y-3">
                ...
                <div className="border-t border-border pt-3">
                  <span className="block text-xs text-muted-foreground mb-2 flex items-center gap-1">
                    <ImageIcon className="w-4 h-4" /> Quick Capture                  </span>
            ...
            <Card className="p-4 flex-1 min-h-0 flex flex-col">
              <div className="flex items-center justify-between mb-3 shrink-0">
                <div className="text-xs text-muted-foreground font-semibold uppercase tracking-wider flex items-center gap-1">
                  <Images className="w-4 h-4" /> {t("agents.screen_gallery")}
                </div>
                <Badge variant="secondary" className="text-[10px]">{screenshotGallery.length}</Badge>
              </div>
              {screenshotGallery.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 overflow-y-auto flex-1 max-h-64 lg:max-h-full">
                  {screenshotGallery.map((item) => (
                    <div key={item.id} className="relative group cursor-pointer rounded-xl overflow-hidden border-2 border-transparent hover:border-primary transition-colors bg-muted"
                      ...
                      <img src={item.data} alt="Thumbnail" className="w-full h-auto aspect-video object-cover" loading="lazy" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-1.5 py-1 text-[9px] text-white opacity-0 group-hover:opacity-100 transition-opacity">
                        {item.timestamp}
                      </div>
                      <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          ...
                          className="bg-black/60 hover:bg-black/80 text-white p-1 rounded text-[9px] transition-colors h-auto"
                        >
                ...
                <div className="text-center py-6 text-muted-foreground/70 text-xs flex-1 flex flex-col items-center justify-center">                  <ImageIcon className="w-4 h-4" />                  <p>{t("agents.screen_no_screenshots")}</p>
                </div>
              )}            </Card>
          </div>        </div>
