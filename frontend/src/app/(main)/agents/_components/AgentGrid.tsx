...
import { avatarInitial, avatarColor, formatUptime } from "./types";
...
export function AgentGrid({ beacons, tagsByAgent, taskCountMap, onSelect }: AgentGridProps) {
  const { t } = useI18n();
  return (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2.5 min-w-0">
                <span className={`w-9 h-9 rounded-xl ${avatarColor(hostname)} flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-sm`}>
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[11px] text-muted-foreground/70 truncate block">{beacon.username || ""}</span>
                </div>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <span className="text-[10px] text-muted-foreground/70 capitalize">{status}</span>
              </div>
            </div>
            <div className="space-y-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                {beacon.integrity && <span className={`px-1 py-0.5 rounded text-[9px] font-semibold ${
              ...
              <div className="flex items-center gap-1.5">
                ...
                {beacon.created_at && <span className="text-[9px] text-muted-foreground/70 ml-1">up {formatUptime(beacon.last_seen || beacon.created_at)}</span>}
              </div>
            </div>
            <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-border/40">
              ...
              <span className="ml-auto text-[10px] text-muted-foreground/70">{t("agents.n_tasks").replace("{n}", String(taskCountMap[id] ?? 0))}</span>
            </div>
          </Card>
        );
      })}
