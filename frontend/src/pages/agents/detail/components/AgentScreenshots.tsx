      ...
      <Card className="mb-4 overflow-hidden border-border/70 bg-card/90 shadow-sm">
        ...
        <div className="px-4 py-3 border-b border-border/70 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link to={`/agents/${agentId}/screen`} className="text-xs text-primary hover:underline">{t("agents.screenshots_open_monitor")} &rarr;</Link>
...
<Button key={fn} variant="ghost" onClick={() => onOpenLightbox(i)} className="group relative aspect-video rounded-lg border border-border/70 bg-muted/30 p-0 h-auto overflow-hidden shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity group-hover:opacity-100">
                  <span className="flex size-8 items-center justify-center rounded-full bg-black/40 text-white backdrop-blur-sm">
        ...
        <Dialog open={true} onOpenChange={onCloseLightbox}>
          <DialogContent className="max-w-5xl bg-transparent border-0 p-0 shadow-none" showCloseButton={false}>
            <div className="relative mx-auto max-w-[92vw] max-h-[88vh]">
              ...
              <Button variant="ghost" size="icon-lg" onClick={onNextLightbox} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-black/50 hover:bg-black/70 text-white backdrop-blur-sm" disabled={lightboxIdx! >= screenshots.length - 1} aria-label={t("agents.screenshots_next")}><ChevronRight className="size-4" /></Button>
              <Button variant="ghost" size="icon" onClick={onCloseLightbox} className="absolute top-3 right-3 rounded-full bg-black/50 hover:bg-black/70 text-white backdrop-blur-sm" aria-label={t("agents.screenshots_close")}><X className="size-4" /></Button>
            </div>
          </DialogContent>
        </Dialog>
)}
