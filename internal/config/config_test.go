...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{"negative session_max_concurrent", func(c *Config) { c.Auth.SessionMaxConcurrent = -1 }, true, "session_max_concurrent"},
		{"zero session_max_concurrent (unlimited)", func(c *Config) { c.Auth.SessionMaxConcurrent = 0 }, false, ""},
		{"positive session_max_concurrent", func(c *Config) { c.Auth.SessionMaxConcurrent = 5 }, false, ""},
		...
		{"session_cipher empty accepted", func(c *Config) { c.Crypto.SessionCipher = "" }, false, ""},
		{"session_cipher aes-gcm accepted", func(c *Config) { c.Crypto.SessionCipher = "aes-gcm" }, false, ""},
		{"session_cipher chacha20 accepted", func(c *Config) { c.Crypto.SessionCipher = "chacha20" }, false, ""},
		{"session_cipher unknown rejected", func(c *Config) { c.Crypto.SessionCipher = "rc4" }, true, "session_cipher"},
