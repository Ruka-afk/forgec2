...
func GenerateCaddyConfig(rc RedirectorConfig) string {
	rc.fillDefaults()
	var b strings.Builder

	b.WriteString("# ForgeC2 Caddy Redirector Configuration\n")
	b.WriteString(fmt.Sprintf("# Generated for domain: %s\n", rc.Domain))
	b.WriteString(fmt.Sprintf("# Backend: %s\n\n", rc.BackendURL))
	...
	b.WriteString(fmt.Sprintf("    # Listen on port %d\n", rc.ListenPort))
	b.WriteString("    \n")

	b.WriteString("    # TLS with automatic HTTPS (or use manual cert)\n")
	if rc.CertPath != "" && rc.KeyPath != "" {
		b.WriteString(fmt.Sprintf("    tls %s %s\n", rc.CertPath, rc.KeyPath))
	} else {
		b.WriteString("    tls {\n")
		b.WriteString("        protocols tls1.2 tls1.3\n")
		b.WriteString("    }\n")
	}
	b.WriteString("    \n")
	...
	b.WriteString("        zone dynamic {\n")
	b.WriteString("            key {remote_host}\n")
	b.WriteString("            events 100\n")
	...
	// Blocked IPs
	for _, ip := range rc.BlockedIPs {
		b.WriteString(fmt.Sprintf("    @blocked_%s remote_ip %s\n", ip, ip))
		b.WriteString("    handle @blocked_" + ip + " {\n")
	...
	// Beacon proxy
	b.WriteString("    # C2 Beacon API\n")
	b.WriteString("    handle_path /api/v1/* {\n")
	...
	// Stage serving
	b.WriteString("    # Stager payload download\n")
	b.WriteString("    handle_path /stage/* {\n")
	b.WriteString(fmt.Sprintf("        reverse_proxy %s\n", rc.BackendURL))
	b.WriteString("    }\n")
	b.WriteString("    \n")
	...
	// Payload hosting
	b.WriteString("    # Hosted payloads\n")
	b.WriteString("    handle_path /payloads/* {\n")
	b.WriteString(fmt.Sprintf("        reverse_proxy %s\n", rc.BackendURL))
	b.WriteString("    }\n")
	b.WriteString("    \n")
	...
	b.WriteString("    handle_path /health {\n")
	b.WriteString(fmt.Sprintf("        reverse_proxy %s\n", rc.BackendURL))
	b.WriteString("    }\n")
	b.WriteString("    handle_path /ready {\n")
	b.WriteString(fmt.Sprintf("        reverse_proxy %s\n", rc.BackendURL))
	b.WriteString("    }\n")
	b.WriteString("    \n")
	...
	// Login rate limited
	b.WriteString("    # Login\n")
	b.WriteString("    handle_path /login {\n")
	b.WriteString(fmt.Sprintf("        reverse_proxy %s\n", rc.BackendURL))
	b.WriteString("    }\n")

	// Static
	b.WriteString("    # Static assets\n")
	b.WriteString("    handle_path /static/* {\n")
	b.WriteString(fmt.Sprintf("        reverse_proxy %s\n", rc.BackendURL))
	b.WriteString("        header Cache-Control \"public, immutable\"\n")
	b.WriteString("    }\n")
	b.WriteString("    \n")
