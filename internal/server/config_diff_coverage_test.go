...
func TestDiffConfigTokenCoverage(t *testing.T) {
	...
	cases := []struct {
		...
		{"crypto.update_signing_key", "hot", func(c *config.Config) { c.Crypto.UpdateSigningKey += "x" }},
		{"crypto.require_release_signature", "hot", func(c *config.Config) { c.Crypto.RequireReleaseSignature = !c.Crypto.RequireReleaseSignature }},
