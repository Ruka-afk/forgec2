...
func TestBuildCache_HashDeterministic(t *testing.T) {
	t.Helper()
	...
	params := buildParams{OS: "windows", Arch: "amd64", Format: "exe", Obfuscate: true}

	hash1 := HashParams(params)
	hash2 := HashParams(params)
	if hash1 == "" {
		t.Fatal("HashParams returned empty string")
	}
	if hash1 != hash2 {
		t.Errorf("HashParams not deterministic: %q != %q", hash1, hash2)
	}

	different := buildParams{OS: "linux", Arch: "amd64", Format: "elf", Obfuscate: false}
	hash3 := HashParams(different)
	if hash1 == hash3 {
		t.Errorf("HashParams produced same hash for different params: %q", hash1)
	}
}
...
func TestBuildCache_StoreAndLookup(t *testing.T) {
	t.Helper()
	dir := t.TempDir()
	bc, err := NewBuildCache(dir, 100, time.Hour)
	if err != nil {
		t.Fatalf("NewBuildCache: %v", err)
	}

	paramsHash := HashParams("test-params-store")
	data := []byte("fake-build-artifact")
	path, err := bc.Store(paramsHash, data, ".exe")
	if err != nil {
	...
	lookupPath, ok := bc.Lookup(paramsHash)
	if !ok {
		t.Fatal("Lookup should hit after Store")
	}
	if lookupPath != path {
		t.Errorf("Lookup path %q != Store path %q", lookupPath, path)
	}
...
func TestBuildCache_Expiry(t *testing.T) {
	t.Helper()
	dir := t.TempDir()
	bc, err := NewBuildCache(dir, 100, 50*time.Millisecond)
	if err != nil {
		t.Fatalf("NewBuildCache: %v", err)
	}

	paramsHash := HashParams("test-params-expiry")
	_, err = bc.Store(paramsHash, []byte("data"), ".bin")
	if err != nil {
		t.Fatalf("Store: %v", err)
	}

	_, ok := bc.Lookup(paramsHash)
	if !ok {
	...
	_, ok = bc.Lookup(paramsHash)
	if ok {
		t.Error("Lookup should miss after TTL expiry")
	}

	stats := bc.Stats()
	if stats.Misses < 1 {
		t.Errorf("expected at least 1 miss after expiry, got %d", stats.Misses)
	}
}
...
func TestWireGuardManager_AddRemovePeer(t *testing.T) {
	t.Helper()
	wg, err := NewWireGuardManager(51820, "10.0.0.1/24")
	if err != nil {
		t.Fatalf("NewWireGuardManager: %v", err)
	}

	pubKeyBytes := make([]byte, 32)
	rand.Read(pubKeyBytes)
	pubKey := base64Encode(pubKeyBytes)

	peer, err := wg.AddPeer(pubKey, "1.2.3.4:51821", []string{"10.0.0.2/32"}, "agent-001")
	if err != nil {
		t.Fatalf("AddPeer: %v", err)
	}
	if peer.PublicKey != pubKey {
		t.Errorf("peer.PublicKey = %q, want %q", peer.PublicKey, pubKey)
	}
	if peer.AgentID != "agent-001" {
		t.Errorf("peer.AgentID = %q, want %q", peer.AgentID, "agent-001")
	}

	got, ok := wg.GetPeer(pubKey)
	if !ok {
		t.Fatal("GetPeer should find added peer")
	}
	if got.Endpoint != "1.2.3.4:51821" {
		t.Errorf("peer.Endpoint = %q, want %q", got.Endpoint, "1.2.3.4:51821")
	}

	peers := wg.ListPeers()
	if len(peers) != 1 {
		t.Errorf("ListPeers count = %d, want 1", len(peers))
	}

	wg.RemovePeer(pubKey)

	_, ok = wg.GetPeer(pubKey)
	if ok {
	...
	peers = wg.ListPeers()
	if len(peers) != 0 {
		t.Errorf("ListPeers count after remove = %d, want 0", len(peers))
	}
}

func TestWireGuardManager_DuplicatePeer(t *testing.T) {
	t.Helper()
	wg, err := NewWireGuardManager(51820, "10.0.0.1/24")
	if err != nil {
		t.Fatalf("NewWireGuardManager: %v", err)
	...
	_, err = wg.AddPeer(pubKey, "1.2.3.4:51821", []string{"10.0.0.2/32"}, "agent-001")
	if err != nil {
		t.Fatalf("first AddPeer: %v", err)
	}

	_, err = wg.AddPeer(pubKey, "1.2.3.4:51822", []string{"10.0.0.3/32"}, "agent-002")
	if err == nil {
		t.Error("expected error when adding duplicate peer, got nil")
	}
}
...
func TestWireGuardManager_HealthCheck(t *testing.T) {
	...
	wg.UpdatePeerStats(pubKey, 1024, 2048, time.Now())
	if !peer.Connected {
		t.Error("peer should be connected after recent handshake")
	}
	if wg.ConnectedPeerCount() != 1 {
		t.Errorf("ConnectedPeerCount = %d, want 1", wg.ConnectedPeerCount())
	}

	wg.UpdatePeerStats(pubKey, 1024, 2048, time.Now().Add(-10*time.Minute))
	if peer.Connected {
		t.Error("peer should be disconnected after stale handshake")
	}
	if wg.ConnectedPeerCount() != 0 {
		t.Errorf("ConnectedPeerCount = %d, want 0", wg.ConnectedPeerCount())
	}

	wg.UpdatePeerStats(pubKey, 1024, 2048, time.Now().Add(-5*time.Second))
	if !peer.Connected {
		t.Error("peer should be connected after recent handshake")
	}
}
